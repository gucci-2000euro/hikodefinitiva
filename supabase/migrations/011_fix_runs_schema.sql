-- ══════════════════════════════════════════════════════════════════════
-- HIKO — Ricrea runs con schema completo
-- (la tabella esistente era incompleta: mancavano distanza_km, durata_sec, ecc.)
-- ══════════════════════════════════════════════════════════════════════
...
CREATE TABLE runs (
  ...
  pace_medio   integer       NOT NULL DEFAULT 0,
  waypoints    jsonb,
...
-- Trigger aggiornamento stats profilo
CREATE OR REPLACE FUNCTION update_profile_after_run()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  UPDATE profiles
  SET km_totali    = km_totali    + NEW.distanza_km,
